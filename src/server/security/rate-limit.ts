...
export function consumeRateLimit(database: AppDatabase, config: AppConfig, policy: RatePolicy, now = Date.now()): RateDecision {
  const keyHash = hmac(config, "rate-limit", `${policy.bucket}:${policy.key}`);
  const transaction = database.sqlite.transaction(() => {
    const current = database.sqlite.prepare("SELECT * FROM rate_limits WHERE key_hash = ?").get(keyHash) as
    ...
    `).run(keyHash, policy.bucket, resetWindow ? now : current?.window_started_at ?? now, attempts, failures, blockedUntil, now);
