...
export function App() {
  ...
  const bootstrap = useCallback(async () => {
    if (isPublic) return;
    try {
      const setup = await api<{ setupRequired: boolean; recoveryAuthorized: boolean }>("/api/setup/status");
  ...
  if (state === "setup") return <SetupView recoveryAuthorized={recoveryAuthorized} onFinished={() => { setRecoveryAuthorized(false); setState("authenticated"); navigate("/files"); }} />;
  if (state === "login") return <LoginView onAuthenticated={(resetRequired) => { if (resetRequired) { setRecoveryAuthorized(true); setState("setup"); navigate("/setup"); } else { setState("authenticated"); navigate("/files"); } }} />;
